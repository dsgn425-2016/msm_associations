class Movie < ActiveRecord::Base
  ####self.table_name =     #### important
  # Movie:
  #  - director_id: must be present
  validates :director_id, :presence => true

  #  - title: must be present; must be unique in combination with year
  validates :title, :presence => true, :uniqueness => { :scope => :year }  ###this makes sure that the same movie with the same name and same year are created (aka, no repeats)
